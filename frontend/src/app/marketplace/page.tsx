  ...
  async function buy(m:any){
    ...
    try{
      ...
      await new Promise<void>((resolve,reject)=>{
        let unsub:any;
        contract.tx.purchaseAccess({gasLimit:gas,value:price,storageDepositLimit:null},m.id)
          .signAndSend(bob,({status,dispatchError}:any)=>{
  ...
  return (
    <div>
      <div className="fade-up" style={{marginBottom:40}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
          <div>
            ...
            <h1 className="display" style={{fontSize:36,fontWeight:800,letterSpacing:"-0.02em",marginBottom:8}}>AI Model Marketplace</h1>
            <p style={{color:"var(--text-secondary)",fontSize:15}}>Purchase access to verified AI models. Pay once, verify infinitely.</p>
          </div>
          <Link href="/register" style={{display:"inline-flex",alignItems:"center",gap:6,padding:"10px 20px",borderRadius:8,background:"rgba(124,58,237,0.15)",border:"1px solid rgba(124,58,237,0.4)",color:"var(--violet-glow)",textDecoration:"none",fontSize:13,fontWeight:600}}>⊕ Register Model</Link>
        </div>
      </div>
      ...
      {loading?(
        ...
        <div style={{textAlign:"center",padding:"100px 0",background:"rgba(13,10,30,0.6)",border:"1px dashed var(--border)",borderRadius:16}}>
          <div style={{fontSize:56,marginBottom:16}}>🤖</div>
          <p style={{color:"var(--text-secondary)",marginBottom:20,fontSize:16}}>No models in the marketplace yet.</p>
          <Link href="/register" style={{display:"inline-flex",padding:"10px 24px",borderRadius:8,background:"rgba(124,58,237,0.15)",border:"1px solid rgba(124,58,237,0.4)",color:"var(--violet-glow)",textDecoration:"none",fontWeight:600}}>Register First Model</Link>
        </div>
      ):(
            ...
            return(
              <div key={m.id} className="model-card fade-up" style={{animationDelay:`${i*0.06}s`}}>
                <div style={{display:"flex",gap:24,alignItems:"flex-start"}}>
                  <div style={{width:56,height:56,borderRadius:14,flexShrink:0,background:"linear-gradient(135deg,rgba(124,58,237,0.3),rgba(79,70,229,0.3))",border:"1px solid rgba(124,58,237,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26}}>🤖</div>
                  <div style={{flex:1}}>
                    ...
                    <p style={{color:"var(--text-muted)",fontSize:14,marginBottom:12,lineHeight:1.5}}>{meta.description||"No description"}</p>
                    <div style={{display:"flex",gap:24}}>
                      {[{label:"ID",val:`#${m.id}`},{label:"Creator",val:m.creator?.slice(0,8)+"..."},{label:"Buyers",val:m.totalPurchases||"0"},{label:"Verifications",val:m.totalVerifications||"0"}].map(d=>(
                        <div key={d.label}>
                  ...
                  <div style={{textAlign:"right",flexShrink:0,minWidth:140}}>
                    <div className="display text-gold" style={{fontSize:26,fontWeight:800,marginBottom:12}}>{fmt(m.price?.replace(/,/g,"")||"0")}</div>
                    {owned?(
                      <Link href={`/verify?id=${m.id}`} style={{display:"block",padding:"10px 20px",borderRadius:8,textAlign:"center",background:"rgba(6,182,212,0.15)",border:"1px solid rgba(6,182,212,0.4)",color:"#67e8f9",textDecoration:"none",fontSize:13,fontWeight:600}}>Verify Inference →</Link>
                    ):(
                      <button onClick={()=>buy(m)} disabled={buying===m.id||m.isActive===false}
                        className="btn-primary"
                        style={{width:"100%",padding:"10px 20px",borderRadius:8,color:"white",fontWeight:600,fontSize:13,border:"none",cursor:"pointer",fontFamily:"Space Grotesk, sans-serif",opacity:buying===m.id?0.7:1}}>
                ...
                {msg?.id===m.id&&(
                  <div style={{marginTop:16,padding:"10px 14px",borderRadius:8,fontSize:13,background:msg.ok?"rgba(16,185,129,0.1)":"rgba(239,68,68,0.1)",border:`1px solid ${msg.ok?"rgba(16,185,129,0.3)":"rgba(239,68,68,0.3)"}`,color:msg.ok?"#34d399":"#f87171"}}>
                    {msg.ok?"✓ ":"✗ "}{msg.text}
