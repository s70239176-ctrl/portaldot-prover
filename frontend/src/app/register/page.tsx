...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setChainReady("ready");
      } catch(e) { setChainReady("failed"); setMessage(e.message); setStatus("error"); }
    })();
  ...
  async function submit(e) {
    ...
    try {
      ...
      const modelHash = "0x"+Array.from(bytes).map(b=>b.toString(16).padStart(2,"0")).join("");
    ...
    <div style={{maxWidth:640,margin:"0 auto"}}>
      ...
      <div className="fade-up-1" style={{
        ...
        <span style={{fontSize:13,color:chainReady==="ready"?"#34d399":chainReady==="failed"?"#f87171":"#a78bfa"}}>
          ...
          {chainReady==="failed" && "Connection failed"}
        </span>
      </div>
        ...
        <div style={{background:"rgba(13,10,30,0.8)",border:"1px solid var(--border)",borderRadius:16,padding:32,display:"flex",flexDirection:"column",gap:20}}>
          ...
          <button type="submit" disabled={status==="loading"||chainReady!=="ready"}
            ...
            style={{width:"100%",padding:"14px",borderRadius:10,color:"white",fontWeight:700,fontSize:15,
              border:"none",cursor:chainReady!=="ready"?"not-allowed":"pointer",
